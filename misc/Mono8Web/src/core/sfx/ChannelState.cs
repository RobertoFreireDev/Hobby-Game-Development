...
{
    private static int SampleRate => AudioSettings.SampleRate;
    ...
    // True once the DSFI has been Play()'d. We keep it playing for the channel's lifetime and never
    // call DynamicSoundEffectInstance.Stop(): the KNI BlazorGL backend has a bug where Stop() nulls
    // its worklet node in PlatformStop() and then dereferences it in DynamicPlatformClearBuffers(),
    // throwing a NullReferenceException out of the frame loop. Instead, "stopping" a sound just
    // ceases buffer submission — the audio worklet keeps its node alive and outputs silence on
    // underrun (see streamProcessor.js), so the voice can be reused by simply submitting again.
    ...
    public void Play(int sfxIndex, SfxData data, int offset, int length)
    {
        ...
        // The DSFI keeps streaming for the channel's lifetime; the first Play() kicks off the
        // worklet, later ones are a no-op while it's already playing. FillBuffer queues this SFX's
        // first buffer; when reusing an already-playing voice a short tail of the previous sound may
        // remain queued (KNI exposes no safe way to flush it — see _voiceStarted), which is a minor
        // audible artifact, not a correctness issue.
        FillBuffer();
        if (!_voiceStarted)
        {
            _dsfi.Play();
    ...
    /// <summary>
    /// Stop playback. This only clears synthesis state so the channel goes silent and can be reused;
    /// it deliberately never calls the DSFI's Stop() (see <see cref="_voiceStarted"/>). Safe to call
    /// from any thread, including the buffer callback.
    /// </summary>
    public void Stop()
    {
        _isPlaying = false;
    ...
    {
        // Idle voice: submit nothing. The worklet keeps its node alive and outputs silence on
        // underrun, so there's no need to feed it (and no reason to allocate) silence buffers.
    ...
    private void AdvanceSampleClock()
    {
        ...
        // Stop() only clears synthesis state (no DSFI call), so it's safe from this callback;
        // the surrounding FillBuffer still submits this buffer's real tail padded with silence.
    ...
    public void Dispose()
    {
        // Dispose directly without Stop(): the KNI BlazorGL Stop() throws (see _voiceStarted), and
        // Dispose() tears the voice down anyway.
        if (!_dsfi.IsDisposed)
        {
            _dsfi.Dispose();
            _voiceStarted = false;
