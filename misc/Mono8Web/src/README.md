...
On launch a short splash screen plays, then the Sprite editor opens. The icon buttons at the **top-right of the menu bar** switch between the four editors: **Sprite**, **Map**, **Sfx** and **Music**. Two buttons at the **far top-left** are **Save cart** and **Load cart** — they export/import the whole `data.mono8` cartridge through a host file dialog (used by the [web build](../README.md); no-ops on the desktop build). The context-sensitive button that follows toggles the full-screen map view in the Map editor and the alternate (tracker) view in the SFX editor.
...
Everything you author in the editors lives in the `data/` folder next to the executable, as plain text you can diff and commit. The whole cartridge is a single file, `data.mono8`, whose sections are introduced by `__{name}__` header lines (PICO-8 style). The **Save** button at the far left of the menu bar writes the whole file at once. `data.icons` is a separate, read-only built-in sheet, and `data.save` is a separate file rewritten by `dset` rather than by the Save button.

| File / Section | Contents |
|---|---|
| `data.mono8` → `__lua__` | Your game's Lua source (up to 524288 characters). |
| `data.mono8` → `__gfx__` | Sprite sheet pixels. |
| `data.mono8` → `__gff__` | Per-sprite flag bits. |
| `data.mono8` → `__atl__` | Which 4×4 sprite blocks are autotiles — 7 lines of 8 `0`/`1` digits. |
| `data.mono8` → `__map__` | Map cells, as two hex digits per cell. |
| `data.mono8` → `__sfx__` | The 64 sound effects. |
| `data.mono8` → `__music__` | The 64 music patterns. |
| `data.icons` | The editors' icon sheet (separate, read-only). |
| `data.save` | The 64 `dget`/`dset` slots, rewritten on every `dset` (separate). |

## Running Your Game

Write your game's logic in **Lua**, in the `__lua__` section at the top of `data.mono8`. Define the PICO-8-style entry points `_init`, `_update(dt)` and `_draw`, and call the API (`spr`, `map`, `btn`, `print`, …) as global functions. The whole `IMono8API` surface is exposed automatically. The Lua is compiled fresh on every `Ctrl+R` by [src/game/LuaGame.cs](src/game/LuaGame.cs) (which hosts a [MoonSharp](https://www.moonsharp.org/) interpreter, so it runs on both the desktop and web heads). It ships with a small demo; replace it to start your own game.
...
A block only behaves as an autotile once you mark it as such, which you do with the autotile button in the **Map Editor** (see below). The marks live in the `__atl__` section of `data.mono8`, one line per block row and one `0`/`1` per block, written with the rest of the project by the menu-bar **Save** button. A missing or short section reads as all-off, so an older project simply loads with no autotiles.
